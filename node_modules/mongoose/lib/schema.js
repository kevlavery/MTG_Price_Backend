 ...
 */
function aliasFields(schema) ***REMOVED***
  for (var path in schema.paths) ***REMOVED***
    ...
    if (alias) ***REMOVED***
      if ('string' === typeof alias && alias.length > 0) ***REMOVED***
        if (schema.aliases[alias]) ***REMOVED***
...
Schema.prototype.path = function(path, obj) ***REMOVED***
  ...
  ***REMOVED***);
  ...
  if (this.paths[path].$isSchemaMap) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (this.paths[path].$isSingleNested) ***REMOVED***
    for (let key in this.paths[path].schema.paths) ***REMOVED***
      this.singleNestedPaths[path + '.' + key] =
          this.paths[path].schema.paths[key];
    ***REMOVED***
    for (let key in this.paths[path].schema.singleNestedPaths) ***REMOVED***
      this.singleNestedPaths[path + '.' + key] =
    ...
    ***REMOVED***);
  ***REMOVED*** else if (this.paths[path].$isMongooseDocumentArray) ***REMOVED***
    this.childSchemas.push(***REMOVED***
      schema: this.paths[path].schema,
      model: this.paths[path].casterConstructor
    ***REMOVED***);
  ***REMOVED*** else if (this.paths[path].path === '_id' && this.paths[path].options && this.paths[path].options.unique) ***REMOVED***
...
Schema.prototype.indexes = function() ***REMOVED***
  ...
  var collectIndexes = function(schema, prefix) ***REMOVED***
    // Ignore infinitely nested schemas, if we've already seen this schema
    // along this path there must be a cycle
    if (schemaStack.indexOf(schema) !== -1) ***REMOVED***
      return;
    ***REMOVED***
    ...
    for (var i = 0; i < keys.length; ++i) ***REMOVED***
      ...
      if ((path instanceof MongooseTypes.DocumentArray) || path.$isSingleNested) ***REMOVED***
        ...
        ***REMOVED***
        ...
        // gh-6113
        if ((path instanceof MongooseTypes.DocumentArray)) ***REMOVED***
          continue;
        ***REMOVED***
      ***REMOVED***
      ...
      if (index !== false && index !== null && index !== undefined) ***REMOVED***
        ...
        options = isObject ? index : ***REMOVED******REMOVED***;
        type = typeof index === 'string' ? index :
          isObject ? index.type :
            false;

        if (type && ~Schema.indexTypes.indexOf(type)) ***REMOVED***
  ...
  ***REMOVED***;
  ...
  function fixSubIndexPaths(schema, prefix) ***REMOVED***
    ...
    for (i = 0; i < len; ++i) ***REMOVED***
      ...
      klen = keys.length;
      newindex = ***REMOVED******REMOVED***;

      // use forward iteration, order matters
      for (j = 0; j < klen; ++j) ***REMOVED***
        key = keys[j];
