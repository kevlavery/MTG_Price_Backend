...
function createData() ***REMOVED***
  Console.create(
    ...
    ***REMOVED***,
    function(err, nintendo64) ***REMOVED***
...
function example() ***REMOVED***
  Game
    .findOne(***REMOVED***name: /^Legend of Zelda/***REMOVED***)
    .exec(function(err, ocinara) ***REMOVED***
      if (err) return done(err);

      console.log('"%s" console _id: %s', ocinara.name, ocinara.consoles[0]);

      // population of existing document
      ocinara.populate('consoles', function(err) ***REMOVED***
        if (err) return done(err);
