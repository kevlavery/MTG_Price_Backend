...
module.exports = function(url, options, callback) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (result.protocol !== 'mongodb:' && result.protocol !== 'mongodb+srv:') ***REMOVED***
    return callback(new Error('Invalid schema, expected `mongodb` or `mongodb+srv`'));
  ***REMOVED***

  if (result.protocol === 'mongodb:') ***REMOVED***
  ...
  ***REMOVED***
  ...
  dns.resolveSrv(srvAddress, function(err, addresses) ***REMOVED***
    ...
    ***REMOVED***
    ...
    let connectionStrings = addresses.map(function(address, i) ***REMOVED***
      if (i === 0) return `$***REMOVED***base***REMOVED***$***REMOVED***address.name***REMOVED***:$***REMOVED***address.port***REMOVED***`;
      else return `$***REMOVED***address.name***REMOVED***:$***REMOVED***address.port***REMOVED***`;
    ***REMOVED***);
    ...
    ***REMOVED***
    ...
    // Default to SSL true
    if (!options.ssl && !result.search) ***REMOVED***
      connectionStringOptions.push('ssl=true');
    ***REMOVED*** else if (!options.ssl && result.search && !result.search.match('ssl')) ***REMOVED***
      connectionStringOptions.push('ssl=true');
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    dns.resolveTxt(result.host, function(err, record) ***REMOVED***
...
function matchesParentDomain(srvAddress, parentDomain) ***REMOVED***
  let regex = /^.*?\./;
  let srv = `.$***REMOVED***srvAddress.replace(regex, '')***REMOVED***`;
  let parent = `.$***REMOVED***parentDomain.replace(regex, '')***REMOVED***`;
  if (srv.endsWith(parent)) return true;
  else return false;
***REMOVED***

function parseHandler(address, options, callback) ***REMOVED***
  let result, err;
  try ***REMOVED***
...
function parseConnectionString(url, options) ***REMOVED***
  // Variables
  let connection_part = '';
  let auth_part = '';
  ...
  if ((result.hostname == null || result.hostname === '') && url.indexOf('.sock') === -1) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!isNaN(parseInt(result.port, 10)) && parseInt(result.port, 10) > 65535) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (result.query) ***REMOVED***
    for (let name in result.query) ***REMOVED***
      if (name.indexOf('::') !== -1) ***REMOVED***
        throw new Error('Double colon in host identifier');
      ***REMOVED***

      if (result.query[name] === '') ***REMOVED***
        throw new Error('Query parameter ' + name + ' is an incomplete value pair');
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***

  if (result.auth) ***REMOVED***
    let parts = result.auth.split(':');
    if (url.indexOf(result.auth) !== -1 && parts.length > 2) ***REMOVED***
      throw new Error('Username with password containing an unescaped colon');
    ***REMOVED***

    if (url.indexOf(result.auth) !== -1 && result.auth.indexOf('@') !== -1) ***REMOVED***
  ...
  ***REMOVED***
  ...
  for (let i = 0; i < strings.length; i++) ***REMOVED***
    let hostString = strings[i];

    if (hostString.indexOf('mongodb') !== -1) ***REMOVED***
      if (hostString.indexOf('@') !== -1) ***REMOVED***
        hosts.push(hostString.split('@').pop());
      ***REMOVED*** else ***REMOVED***
        hosts.push(hostString.substr('mongodb://'.length));
      ***REMOVED***
    ***REMOVED*** else if (hostString.indexOf('/') !== -1) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (connection_part.indexOf('@') !== -1) ***REMOVED***
    auth_part = connection_part.split('@')[0];
    connection_part = connection_part.split('@')[1];
  ***REMOVED***

  // Check there is not more than one unescaped slash
  if (connection_part.split('/').length > 2) ***REMOVED***
    throw new Error(
      "Unsupported host '" +
  ...
  ***REMOVED***
  ...
  if (connection_part.indexOf('.sock') !== -1) ***REMOVED***
    if (connection_part.indexOf('.sock/') !== -1) ***REMOVED***
      dbName = connection_part.split('.sock/')[1];
      // Check if multiple database names provided, or just an illegal trailing backslash
      if (dbName.indexOf('/') !== -1) ***REMOVED***
        if (dbName.split('/').length === 2 && dbName.split('/')[1].length === 0) ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED*** else if (connection_part.indexOf('/') !== -1) ***REMOVED***
    // Check if multiple database names provided, or just an illegal trailing backslash
    if (connection_part.split('/').length > 2) ***REMOVED***
      if (connection_part.split('/')[2].length === 0) ***REMOVED***
        throw new Error('Illegal trailing backslash after database name');
      ***REMOVED***
      throw new Error('More than 1 database name in URL');
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (auth[0] !== encodeURIComponent(user)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (auth[1]) ***REMOVED***
    ...
    if (auth[1] !== encodeURIComponent(pass)) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (auth.length === 2) object.auth = ***REMOVED*** user: auth[0], password: auth[1] ***REMOVED***;
  ...
  let servers;
  let compression;
  let serverOptions = ***REMOVED*** socketOptions: ***REMOVED******REMOVED*** ***REMOVED***;
  let dbOptions = ***REMOVED*** read_preference_tags: [] ***REMOVED***;
  let replSetServersOptions = ***REMOVED*** socketOptions: ***REMOVED******REMOVED*** ***REMOVED***;
  let mongosOptions = ***REMOVED*** socketOptions: ***REMOVED******REMOVED*** ***REMOVED***;
  ...
  if (url.match(/\.sock/)) ***REMOVED***
    ...
    servers = [***REMOVED*** domain_socket: domainSocket ***REMOVED***];
  ***REMOVED*** else ***REMOVED***
    // Split up the db
    hostPart = connection_part;
    // Deduplicate servers
    let deduplicatedServers = ***REMOVED******REMOVED***;

    // Parse all server results
    servers = hostPart
      .split(',')
      .map(function(h) ***REMOVED***
        let _host, _port, ipv6match;
        //check if it matches [IPv6]:port, where the port number is optional
        if ((ipv6match = /\[([^\]]+)\](?::(.+))?/.exec(h))) ***REMOVED***
          _host = ipv6match[1];
        ...
        ***REMOVED***
        ...
        return ***REMOVED*** host: _host, port: _port ***REMOVED***;
      ***REMOVED***)
      .filter(function(x) ***REMOVED***
        return x != null;
      ***REMOVED***);
  ***REMOVED***
  ...
  urlOptions.forEach(function(opt) ***REMOVED***
    ...
    switch (name) ***REMOVED***
      ...
      case 'readConcernLevel':
        dbOptions.readConcern = ***REMOVED*** level: value ***REMOVED***;
        break;
      ...
      case 'authMechanism':
        if (value === 'GSSAPI') ***REMOVED***
          ...
          if (object.auth == null) ***REMOVED***
            ...
            object.auth = ***REMOVED*** user: urlDecodeAuthPart, password: null ***REMOVED***;
          ***REMOVED*** else ***REMOVED***
            object.auth.user = decodeURIComponent(object.auth.user);
          ***REMOVED***
        ***REMOVED*** else if (value === 'MONGODB-X509') ***REMOVED***
          object.auth = ***REMOVED*** user: decodeURIComponent(authPart) ***REMOVED***;
        ***REMOVED***
      ...
      case 'compressors':
        ***REMOVED***
          compression = serverOptions.compression || ***REMOVED******REMOVED***;
          let compressors = value.split(',');
          if (
            !compressors.every(function(compressor) ***REMOVED***
      ...
      case 'zlibCompressionLevel':
        ***REMOVED***
          ...
          if (zlibCompressionLevel < -1 || zlibCompressionLevel > 9) ***REMOVED***
      ...
      default:
        ***REMOVED***
          ...
          logger.warn(`$***REMOVED***name***REMOVED*** is not supported as a connection string option`);
        ***REMOVED***
        break;
    ***REMOVED***
  ***REMOVED***);
  ...
  ***REMOVED***
  ...
  if (!dbOptions.readPreference) ***REMOVED***
