...
* or an object with the following properties:
  - `indent`: *****REMOVED***Number***REMOVED***** the amount of indentation to use. Default is `0` when an object is returned.
  - `character`: *****REMOVED***String***REMOVED***** the character to use for indentation. Default is `''` (empty string) when an object is returned.
  - `prefix`: *****REMOVED***String***REMOVED***** leading characters to use at the beginning of each line. `''` (empty string) when an object is returned.
...
function centerAlign(len, longest, line, lines) ***REMOVED***
...
```js
align(text, function (len, max, line, lines) ***REMOVED***
